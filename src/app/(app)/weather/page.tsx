...
import { weatherData, DailyForecast, getIconForCondition } from '@/lib/weather-data';
import { Thermometer, Umbrella, Wind, Sunrise, Sunset, Clock, Droplets, Sun, Cloud, CloudRain, CloudLightning } from 'lucide-react';
...
export default function WeatherPage() {
  ...
  const currentHour = today.hourly.find(h => new Date(h.time).getHours() === new Date().getHours()) || today.hourly[0];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
            <HourlyForecastChart data={weatherData.daily.flatMap(d => d.hourly).slice(0, 24)} />
        </CardContent>
      </Card>
            ...
            <div key={index} className="flex items-center justify-between p-3 rounded-lg hover:bg-muted/50 transition-colors">
              ...
              <div className="w-24 h-2 bg-muted rounded-full overflow-hidden">
                <div 
                    ...
                    style={{ 
                        width: `${((day.temp.max - day.temp.min) / (weatherData.maxTemp - weatherData.minTemp)) * 100}%`,
                        marginLeft: `${((day.temp.min - weatherData.minTemp) / (weatherData.maxTemp - weatherData.minTemp)) * 100}%`
                    }}
                />
              </div>
